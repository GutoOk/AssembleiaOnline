...
interface CreateAssemblySheetProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
}

export function CreateAssemblySheet({ open, onOpenChange }: CreateAssemblySheetProps) {
  ...
  const onSubmit = async (values: z.infer<typeof assemblySchema>) => {
    ...
    try {
        ...
        onOpenChange(false);
        router.refresh(); // Refresh dashboard to show new assembly
    } catch (error) {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent className="w-full sm:max-w-xl flex flex-col p-0">
            <SheetHeader className="p-6 pb-4">
                <SheetTitle>Criar Nova Assembleia</SheetTitle>
                <SheetDescription>Preencha os detalhes abaixo para agendar uma nova assembleia.</SheetDescription>
            </SheetHeader>
            <Separator />
            <ScrollArea className='flex-1'>
                <div className='p-6'>
                    <Form {...form}>
                        <form id="create-assembly-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                            <FormField
                            ...
                            render={({ field }) => (
                                ...
                                <FormControl>
                                    ...
                                    <div className="flex items-start gap-2 rounded-md border p-3">
                                        <RadioGroupItem value="all_verified_members" id="access-all-sheet" className="mt-1" />
                                        <Label htmlFor="access-all-sheet" className="space-y-1">
                                    ...
                                    <div className="flex items-start gap-2 rounded-md border p-3">
                                        <RadioGroupItem value="restricted_email_list" id="access-restricted-sheet" className="mt-1" />
                                        <Label htmlFor="access-restricted-sheet" className="space-y-1">
            ...
            <SheetFooter className='p-4 border-t'>
                ...
                <Button type="submit" form="create-assembly-form" disabled={isSubmitting}>
